...
public class AdminSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Autowired
//	@Qualifier("com.example.service.AdminDetailServiceImpl")
	private AdminDetailServiceImpl adminDetailsServiceImpl;
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.antMatcher("/adminInstructor")
		    .authorizeRequests() // 認可に関する設定
			.antMatchers("/adminInstructor").permitAll() //「/」などのパスは全てのユーザに許可
		...
		http.formLogin() // ログインに関する設定
			...
			.defaultSuccessUrl("/adminInstructor/instructor_list", false) // 第1引数:デフォルトでログイン成功時に遷移させるパス
